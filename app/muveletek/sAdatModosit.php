...
if(!isset($_SESSION['felh_id'])){
    ...
    if(isset($_POST['mentes'])){
        ...
        if(!empty($_POST['rjelszo']) && !empty($_POST['jelszo']) && !empty($_POST['jelszom'])){
            if(password_verify($_POST['rjelszo'], $sor['jelszo'])){
        ...
        if(isset($hibak)){
            ...
            if(isset($mjelszo)){
                $hash = password_hash($mjelszo, PASSWORD_DEFAULT);
                mysqli_query($dbconn, "UPDATE felhasznalok SET jelszo = '{$hash}' WHERE felhasznalo_id = {$_SESSION['felh_id']}");
            }
            if(isset($ujkep)){
